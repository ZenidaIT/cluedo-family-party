...
const PlayerItem = forwardRef(({ player, index, onRemove, style, className, dragProps, listeners }, ref) => {
    return (
        <div 
            ref={ref} 
            style={style} 
            className={`flex items-center gap-4 bg-white border border-slate-200 p-4 rounded-xl shadow-sm hover:border-indigo-300 hover:shadow-md transition group animate-in zoom-in-95 duration-200 ${className}`}
        >
            <div {...dragProps} {...listeners} className="cursor-grab active:cursor-grabbing p-1 -ml-1 text-slate-300 group-hover:text-indigo-400 touch-none">
