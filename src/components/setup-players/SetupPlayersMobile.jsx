...
const SetupPlayersMobile = ({
    ...
    return (
        <div className="w-full bg-white flex flex-col h-[85vh] rounded-2xl shadow-xl overflow-hidden">
                ...
                <div className="flex items-center justify-between">
                     <div className="flex items-center gap-3">
                        ...
                        <h2 className="font-bold text-lg text-slate-800 flex items-center gap-2">
                            ...
                            {isStandalone ? 'Gestione Giocatori' : (
                                activeTab === 'library' ? 'Rubrica' : 'Squadra'
                            )}
                        </h2>
                     </div>
                     ...
                     {!isStandalone && (
                         <div className="flex bg-slate-100 rounded-lg p-1">
                             <button onClick={() => setActiveTab('library')} className={`px-3 py-1 rounded-md text-sm font-bold transition ${activeTab === 'library' ? 'bg-white shadow text-indigo-600' : 'text-slate-500'}`}>
                                 Tutti
                             </button>
                             <button onClick={() => setActiveTab('squad')} className={`px-3 py-1 rounded-md text-sm font-bold transition ${activeTab === 'squad' ? 'bg-white shadow text-indigo-600' : 'text-slate-500'}`}>
                    ...
                    <div className="space-y-2">
                        {players.length === 0 ? (
                            <div className="text-center py-10 text-slate-400">
                                ...
                                <button onClick={() => setActiveTab('library')} className="text-indigo-500 font-bold mt-2">Vai alla rubrica</button>
                            </div>
                        ) : (
                             players.map((p, idx) => (
                                <div key={p.id} className="flex items-center gap-3 bg-white border border-slate-200 p-3 rounded-xl shadow-sm">
                                    <div className={`w-8 h-8 rounded-full ${PLAYER_COLORS[p.colorIdx]?.class || 'bg-gray-400'} shadow-sm border border-white flex items-center justify-center font-bold text-white text-xs`}>
                                        {idx + 1}
                                    </div>
                                    <span className="font-bold text-slate-800 text-sm flex-1 truncate">{p.name}</span>
                                    <button onClick={() => removeFromSquad(p.id)} className="p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-lg">
                                        <X size={20}/>
                                    </button>
                                </div>
                            ))
                        ...
                        {players.length > 0 && (
                            <div className="sticky bottom-0 pt-4 pb-2 bg-gradient-to-t from-slate-50 to-transparent">
                                <button onClick={onStartGame} className="w-full bg-indigo-600 text-white py-3 rounded-xl font-bold shadow-lg shadow-indigo-200 flex items-center justify-center gap-2">
                                    <Check size={20}/> Gioca
                                </button>
                            </div>
                        )}
                        ...
                        {filteredLibrary.map(sp => {
                            ...
                            return (
                                <React.Fragment key={sp.id}>
                                    ...
                                    >
                                       ...
                                       <div className="flex items-center gap-1">
                                            <button onClick={(e) => { e.stopPropagation(); startEditing(sp); }} 
                                                className={`p-2 rounded-full transition ${inSquad ? 'text-indigo-400 hover:bg-indigo-100' : 'text-slate-300 hover:text-indigo-500'}`}>
                                                <Pencil size={16}/>
                                            </button>
                                       </div>
                                   </div>
                                   ...
                                   {isEditingThis && (
                                       <div className="mt-2 mb-4 pl-4 border-l-4 border-indigo-300 animate-in slide-in-from-left-4">
                                            <PlayerEditForm 
